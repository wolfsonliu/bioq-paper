#!/usr/bin/env python3
"""rfantibody — 3x3 grid of interaction-pAE histograms (one PDF).

One histogram per design target (green = pass at pAE < 10, grey = fail, amber
dashed threshold). All nine panels share a single binning (``[2, 26)`` in unit
steps) so they are directly comparable. Unused grid cells (e.g. with a
``--target`` subset) are left blank.

Styled to ``docs/plotting-style-guide.md`` (Arial, brand palette, tinted panels,
soft grid, dropped top/right spines, left-aligned bold titles); tight-cropped PDF
width stays below 16 cm.

    python3 plot_pae.py                       # all nine targets in one PDF
    python3 plot_pae.py --target HIV_Env,TcdB # a subset (blank cells hidden)
    python3 plot_pae.py --out /tmp/rfantibody.pdf     # custom output path
"""
...
# Shared x-range for every panel so the histograms are directly comparable
# (observed interaction_pae spans ~2.4..24 across the campaign).
...
def _draw_pae_hist(ax, pae: list[float], thr: float, title: str) -> None:
    ...
    if not finite:
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.text(0.5, 0.5, "no pAE data", ha="center", va="center",
...
def build_figure(valid: list[dict]) -> plt.Figure:
    ...
    for i, f in enumerate(valid):
        ...
        _draw_pae_hist(ax, _load_pae(f["target"]), thr, _label(f["target"]))
    ...
    fig.suptitle("RFantibody de novo design via bioq \u2014 interaction-pAE "
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(FIGURES / "rfantibody_pae.pdf"),
